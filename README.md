...
`vendor/imgui_md` isn't pristine upstream - it has two local fixes on top of whatever commit the submodule points at. If you re-clone the submodule fresh from upstream, reapply both (or better: commit them inside the submodule itself and push to your fork, so they travel with the pointer instead of living as an uncommitted diff):
